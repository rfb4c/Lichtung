 ...
 * 才能把「两个 judge 同时判错同一条」的相关性压下来。这条要写进论文的方法段。
 *
 * 与 Judge A 对齐的两点：
 *   ・读**同一份** rubric 与**同一份** JSON Schema 文件，不为某一家单独改判据
 ...
 * 而这个字段会原样进 annotations.json 的 meta，进而进论文——必须是核实过的。
 */
  ...
  if (!model) {
    throw new Error(
      '缺少环境变量 OPENAI_JUDGE_MODEL。\n' +
        ...
        '不要凭记忆填——这个值会进 annotations.json 并被论文引用。',
    );
  }
  return model;
}
    ...
    async annotate(input: JudgeInput): Promise<JudgeVerdict> {
      const completion = await client.chat.completions.create({
        ...
        messages: [
          ...
          { role: 'user', content: renderReport(input) },
        ],
        response_format: {
          type: 'json_schema',
      ...
      return {
        reportId: input.id,
        ...(JSON.parse(message.content) as Omit<JudgeVerdict, 'reportId'>),
      };
    },
  };
}
