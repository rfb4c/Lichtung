...
——那份文档汇总了两条管线现在能支撑哪些主张、口径红线、以及不能写的东西
**开写前有两件事挡着**（均记在该文档 § 0）:
　① Path B 复现校验没跑 —— `npm run match:verify -- --confirm-spend`（再 56 次调用）。
　　 跑出来之前不能声称任何重跑稳定性数字
　② 裁决写不写回 —— app-data.json 仍是人工的 11/28，管线产出 15/28 未写回。
　　 不写回就只能说「管线产出与人工挂载有 19 条差异」，不能说 demo 展示的是管线产出
**管线现状**: Path A csScore 28/28 已落地；Path B 已改为两层（读法①）并实跑，
56 次调用零失败，精确层 κ=0.667  95% CI [0.322, 1.000]（上界截断，原值 1.011；分母 18）
**证据**: `docs/private/evidence/match-run-2026-08-28/`（含事实卡与复算脚本）
