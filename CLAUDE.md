...
**当前版本**: v0.4.6 (三条路径均已实装；Path A 管线已实跑；Path B 匹配管线改为两层，待用新判据重跑)
...
**最后同步**: 2026-08-28
**当前任务**: 下个 session **开始写论文**。先读 `docs/private/交接-论文写作.md`
——那份文档汇总了两条管线能支撑哪些主张、口径红线、以及不能写的东西。
**没有待决问题挡着了**：两条管线均已实跑，复现校验已跑，裁决已写回
**Path B 现状**: 两层管线（精确层模型判命题对齐 / 兜底层按 topicId 查表，不经模型）
　实跑 56 次零失败；精确层 κ=0.667  95% CI [0.322, 1.000]（上界截断，原值 1.011；分母 18）
　复现校验：逐条 97.2%、裁决产出 96.4%，但重跑的 κ 是 0.778
　—— **单轮 κ 不能当管线属性**，这是论文最重要的一条口径红线
　挂载已写回 app-data.json：15/28（精确层 6 · 兜底层 9），demo 渲染的就是管线产出
**⚠️ demo 案例**: 用 `rp_abortion_005`，不要用 `rp_abortion_003`
　（后者写回后无图，且是全语料唯一一篇两轮结果不同的报道）
**Path A 现状**: csScore 28/28 已落地
**证据**: `docs/private/evidence/match-run-2026-08-28/`（18 个文件，含事实卡与复算脚本）
