...
| Path A 标注管线 | ✅ 已实跑 | 2026-08-25 首次实跑，28 篇 × 2 judge 零失败，`csScore` 已写回 app-data.json |
| 民调库扩展 | 🟡 4 条入库 | 判据、字段、图表、校验脚本均就绪；3 条按判据排除并留档。扩库等人工核实的新数据 |
| 报道→民调匹配管线 | 🟡 代码完成 | 双评审管线可跑，**尚未实跑**；挂载目前仍是人工填写的 11/28 |
| 配图溯源 | ✅ 28/28 | 全部实测可加载，27/28 与标注来源一致 |
| Demo 部署 | 🔵 未开始 | — |
| News Agent | 🔵 未开始 | — |

**图例**: ✅ 已实装 | 🟡 进行中 | 🔵 未开始 | 🔴 受阻

**当前焦点**: 实跑匹配管线，让 `pollingDataId` 由人工填写变为管线产出。
