...
- [ ] **卡片溢出修复** — `mainColumn` 改为响应式宽度，消除手机端横向滚动
- [ ] **按键区左对齐** — `actions` 从 `space-between` 改为 `flex-start + gap`
- [ ] **平板侧边栏** — 768-1023px 显示 X 风格图标侧边栏（68px），< 768px 保留底部导航
- [ ] **UserMenu 平板适配** — 窄侧边栏下隐藏文字，仅显示头像
