...
| 功能模块 | 状态 | 说明 |
|---------|------|------|
| Path B 温和多数可视化 | ✅ 已实装 | 按需展开的分布图表层。**覆盖不全**：4 个议题只有 2 个有民调 |
| Path C 交叉身份 | ✅ 已实装 | 常驻的身份标签展示层，无开关 |
| Path A Feed 排序干预 | ✅ 已实装 | 连续 csScore 位移 + 间隔调整，可在界面切换排序模式 |
| Path A 标注管线 | 🟡 代码完成 | 双评审管线可跑，**尚未实跑**；`csScore` 目前回退自旧的二值标注 |
| 民调库扩展 | 🔴 受阻 | 判据、字段、图表、校验脚本均已就绪，等人工核实的民调数据入库 |
| 配图溯源 | 🟡 22/28 | 6 条待处理，其中 2 条必须换（用的是搜索引擎图片缓存） |
| Demo 部署 | 🔵 未开始 | — |
| News Agent | 🔵 未开始 | — |

**图例**: ✅ 已实装 | 🟡 进行中 | 🔵 未开始 | 🔴 受阻

**当前焦点**: 实跑标注管线，让 `csScore` 由二值回退变为管线产出。
